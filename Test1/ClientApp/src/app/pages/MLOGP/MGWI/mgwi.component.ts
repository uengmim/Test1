...
*/
    ...
    this.weightEnterRegister = {
      ...
      onClick: async (e: any) => {
        ...
        if (!formresult.isValid || !result.isValid) {
          ...
          //  console.log(this.weightStartData.ZGW_NAME1)
          //  var status = ""
          //  if (this.inProgress) {
          //    status = "계근 입력 상태"
          //  } else {
  ...
  //이력데이터로드
  public async orderdataLoad(dataService: ImateDataService, thisObj: MGWIComponent) {
  ...
  //이력데이터로드
  public async carDataLoad(dataService: ImateDataService, thisObj: MGWIComponent) {
 ...
 * */
  stopMonitoring() {
    try {
      ...
      console.error('오류 발생(모니터링 중지):' + err);
    }
  }
