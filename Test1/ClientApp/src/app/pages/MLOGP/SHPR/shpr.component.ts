 ...
 * */
    ...
    this.shipmentProcessing = {
      ...
      onClick: async () => {
        ...
        //임가공
          ...
          //여기부터 저장로직
          //this.imOrderList.forEach(async (row: ZMMT1320Model) => {
          //  this.orderGridData.push(new ZSDS6410Model(row.VBELN, "", "", "", "", "", "", "", row.SC_R_DATE, row.IDNRK, row.MAKTX, row.SC_R_MENGE, row.SC_L_MENGE,
  ...
  public async dataLoad(thisObj: SHPRComponent) {
    ...
    if (thisObj.selectData2 !== "9999") {
      var zsdif = new ZSDIFPORTALSAPLE028SndModel("", "", "", "", "", "", "", this.startDate, this.endDate, "", "", this.selectData2, "", "", "", "40", zsds6410);
  ...
  async releaseButton(e: any) {
    ...
    if (await confirm("출고처리 하시겠습니까?", "알림")) {
      if (selectData[0].ZMENGE3 === null || selectData[0].ZMENGE3 === undefined || selectData[0].ZMENGE3 === "") {
        alert(`출고수량은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].WADAT_IST === null || selectData[0].WADAT_IST === undefined || selectData[0].WADAT_IST === "") {
        alert(`출고전기일자는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].MATNR === null || selectData[0].MATNR === undefined || selectData[0].MATNR === "") {
        alert(`자재명은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].Z3PARVW === null || selectData[0].Z3PARVW === undefined || selectData[0].Z3PARVW === "") {
        alert(`1차운송사는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZSHIPMENT_NO === null || selectData[0].ZSHIPMENT_NO === undefined || selectData[0].ZSHIPMENT_NO === "") {
        alert(`배차번호는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZSHIPMENT_DATE === null || selectData[0].ZSHIPMENT_DATE === undefined || selectData[0].ZSHIPMENT_DATE === "") {
        alert(`배차일자는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZPALLTP === null || selectData[0].ZPALLTP === undefined || selectData[0].ZPALLTP === "") {
        alert(`파레트유형은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZPALLETQTY === null || selectData[0].ZPALLETQTY === undefined || selectData[0].ZPALLETQTY === "") {
        alert(`파레트수량은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZUNLOAD === null || selectData[0].ZUNLOAD === undefined || selectData[0].ZUNLOAD === "") {
        alert(`하차정보는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZVKAUS === null || selectData[0].ZVKAUS === undefined || selectData[0].ZVKAUS === "") {
        alert(`용도는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZCARTYPE === null || selectData[0].ZCARTYPE === undefined || selectData[0].ZCARTYPE === "") {
        alert(`화물차종은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZCARNO === null || selectData[0].ZCARNO === undefined || selectData[0].ZCARNO === "") {
        alert(`차량번호는 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZDRIVER === null || selectData[0].ZDRIVER === undefined || selectData[0].ZDRIVER === "") {
        alert(`운전기사명은 필수값입니다.`, "알림");
        return;
      }
      else if (selectData[0].ZPHONE === null || selectData[0].ZPHONE === undefined || selectData[0].ZPHONE === "") {
        alert(`전화번호는 필수값입니다.`, "알림");
        return;
      }
  ...
  public async release() {
    ...
    if (checkVSTEL === undefined) {
      selectData.forEach(async (row: ZSDS6400Model) => {
        var zsds6400 = new ZSDS6400Model(row.VBELN, row.POSNR, "I", row.ZSAPSTATUS, row.KZPOD, row.VGBEL,
 ...
 */
  ...
  async TeansactionPrint(e: any) {
    ...
    {
      "dbTitle": this.appConfig.dbTitle,
      "dateFrom": selectData[0].VDATU,
      "dateTo": selectData[0].VDATU,
      "spart": "10",
      "erdat": selectData[0].ERDAT,
      "matnr": selectData[0].MATNR,
      "kunnr": selectData[0].KUNAG,
      "kunwe": selectData[0].KUNWE,
       "mandt": this.appConfig.mandt
    };

    setTimeout(() => { this.reportViewer.printReport("specificationOnTransaction", params) });
  }
  ...
  public async createOrder() {
    ...
    var zsds6420Model = new ZSDS6420Model(selectData[0].VBELN, selectData[0].POSNR, selectData[0].ZSEQUENCY, selectData[0].VRKME, selectData[0].ZMENGE4,
      selectData[0].ZMENGE3, selectData[0].WADAT_IST, selectData[0].Z3PARVW, selectData[0].Z4PARVW, selectData[0].ZCARTYPE,
      selectData[0].ZCARNO, selectData[0].ZDRIVER, selectData[0].ZDRIVER1, selectData[0].ZPHONE, selectData[0].ZPHONE1,
      selectData[0].ZVKAUS, selectData[0].ZUNLOAD, selectData[0].ZSHIPSTATUS, selectData[0].ZSHIPMENT_NO, selectData[0].ZSHIPMENT_DATE,
      selectData[0].ZPALLTP, selectData[0].ZPALLETQTY, selectData[0].ZCONFIRM_CUT, selectData[0].ZTEXT, selectData[0].MTY, selectData[0].MSG);
