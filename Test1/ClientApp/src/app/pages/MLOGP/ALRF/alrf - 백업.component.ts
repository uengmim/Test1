...
import { Service, CarSeq } from '../ALRF/app.service';
import {
  DxDataGridComponent,
  DxDateBoxModule,
  DxFormComponent,
...
export class ALRFComponent {
  ...
  @ViewChild('dd07t2Entery', { static: false }) dd07t2Entery!: TablePossibleEntryComponent;
  @ViewChild('dd07tCarEntery', { static: false }) dd07tCarEntery!: TablePossibleEntryComponent;
  @ViewChild('dd07t2CarEntery', { static: false }) dd07t2CarEntery!: TablePossibleEntryComponent;
  @ViewChild('tvlvEntery', { static: false }) tvlvEntery!: TablePossibleEntryComponent;
  @ViewChild('tvlv2Entery', { static: false }) tvlv2Entery!: TablePossibleEntryComponent;
  @ViewChild('zpalEntery', { static: false }) zpalEntery!: TablePossibleEntryComponent;
  @ViewChild('zpal2Entery', { static: false }) zpal2Entery!: TablePossibleEntryComponent;
  ...
  @ViewChild('zcarno2ModiCodeEntery', { static: false }) zcarno2ModiCodeEntery!: TablePossibleEntryComponent;
  /* Entry  선언 */
  //출하지점
  ...
  zunload2Value!: string | null;
  //용도
  vkausValue!: string | null;
  vkaus2Value!: string | null;
  //용도
  zpalValue!: string | null;
  zpal2Value!: string | null;
  //화물차종
  zcarValue!: string | null;
  zcar2Value!: string | null;
  //2차운송사
  ...
  constructor(private appConfig: AppConfigService, private dataService: ImateDataService, service: Service, private appInfo: AppInfoService, private imInfo: ImateInfo, private authService: AuthService) {
    ...
    this.saveButtonOptions2 = {
      ...
      onClick: () => {
        ...
        if (this.addFormData.ZDRIVER == "") {
          alert("기사명1을 입력해주세요.", "알림");
          return;
        }
        if (this.addFormData.ZPHONE == "") {
          alert("전화번호1을 입력해주세요..", "알림");
          return;
        }

        if (this.popupData2.possible < this.addFormData.ZMENGE4) {
          alert("분할가능수량을 넘을 수 없습니다.", "알림");
          this.addFormData.ZMENGE4 = this.popupData2.possible;
          return;
        }
    ...
    this.saveButtonOptions4 = {
      ...
      onClick: () => {
        ...
        if (this.addFormData2.ZDRIVER == "") {
          alert("기사명1을 입력해주세요.", "알림");
          return;
        }
        if (this.addFormData2.ZPHONE == "") {
          alert("전화번호1을 입력해주세요..", "알림");
          return;
        }
 ...
 */
  ...
  orderDBClick2(e: any) {
    ...
    //하차방법
    this.zunload2Value = this.selectGrid2Data[0].ZUNLOAD;
    //용도
    this.vkaus2Value = this.selectGrid2Data[0].ZVKAUS;
    //파렛트유형
    this.zpal2Value = this.selectGrid2Data[0].ZPALLTP;
    //화물차종
    this.zcar2Value = this.selectGrid2Data[0].ZCARTYPE;
    //차량번호
    this.zcarno2ModiValue = this.selectGrid2Data[0].ZCARNO;
  ...
  //하차
  onZunload2CodeValueChanged(e: any) {
    setTimeout(() => {
      this.addFormData2.ZUNLOAD = e.selectedValue;
    });
  }
  //용도
  onZvkaus2CodeValueChanged(e: any) {
    setTimeout(() => {
      this.addFormData2.ZVKAUS = e.selectedValue;
    });
  }
  //파레트유형
  onZpalltp2CodeValueChanged(e: any) {
    setTimeout(() => {
      this.addFormData2.ZPALLTP = e.selectedValue;
    });
  }
  //화물차종
  onZcartype2CodeValueChanged(e: any) {
    setTimeout(() => {
  ...
  onZcarno1CodeValueChanged(e: any) {
    setTimeout(() => {
      this.addFormData.ZCARNO = e.selectedItem.ZCARNO;
      ...
      this.addFormData.ZCARTYPE = this.zcarValue
    });
  }
  ...
  onZcarno2CodeValueChanged(e: any) {
    setTimeout(() => {
      this.addFormData2.ZCARNO = e.selectedItem.ZCARNO;
      this.addFormData2.ZDRIVER = e.selectedItem.ZDRIVER;
      this.addFormData2.ZPHONE = e.selectedItem.ZPHONE;
      this.zcar2Value = e.selectedItem.ZCARTYPE;
      this.addFormData2.ZCARTYPE = this.zcarValue
    });
  }
